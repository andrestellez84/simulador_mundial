...
# Pega aquí EXACTAMENTE el bloque de texto gigante de tu primer mensaje.
# (He puesto las primeras y últimas líneas como ejemplo, asegúrate de pegar las 495)
TABLA_OFICIAL = """
...
def main():
    # El orden exacto de las columnas de ganadores en la tabla oficial
    WINNERS = ["1A", "1B", "1D", "1E", "1G", "1I", "1K", "1L"]
    
    annex_c = []
    
    # Expresión regular para extraer los datos de cada línea:
    # 1. \d+          -> Ignora el número inicial de la fila (ej. "194")
    # 2. ([A-L]{8})   -> Captura los 8 grupos disponibles (ej. "ADEFGIJL")
    # 3. ((?:3[A-L]){8}) -> Captura la cadena corrida de cruces (ej. "3E3G3J3D3A3F3L3I")
    pattern = re.compile(r'\d+([A-L]{8})((?:3[A-L]){8})')
    
    # Buscar todas las coincidencias válidas en el texto proporcionado
    matches = pattern.findall(TABLA_OFICIAL)
    
    for grupos_str, cruces_str in matches:
        # Convertir el string "EFGHIJKL" en una lista ["E", "F", "G", "H", "I", "J", "K", "L"]
        groups_list = list(grupos_str)
        
        # Convertir la cadena "3E3J3I..." en una lista ["3E", "3J", "3I", ...] separando cada 2 caracteres
        cruces_list = [cruces_str[i:i+2] for i in range(0, len(cruces_str), 2)]
        
        # Mapear cada ganador con su oponente usando el orden de las columnas
        matchups = {WINNERS[i]: cruces_list[i] for i in range(len(WINNERS))}
    ...
    # Validar que se procesaron las 495 líneas (útil para detectar si copiaste mal el texto)
    if len(annex_c) != 495:
        print(f"⚠️ ADVERTENCIA: Se procesaron {len(annex_c)} combinaciones en lugar de 495. Revisa el texto pegado.")
        
    # Guardar la estructura en el archivo JSON
    out_dir = Path("src/worldcup_sim/data/bracket")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / "annex_c.json"
    
    with open(out_file, "w") as f:
        json.dump(annex_c, f, indent=2)
        
    print(f"✅ Generadas exitosamente {len(annex_c)} combinaciones estáticas en {out_file}")
